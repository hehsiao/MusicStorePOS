...
public class PurchaseController implements ActionListener, ExceptionListener
{
	...
	public PurchaseController(AMSView AMS)
	{
		this.AMS = AMS;
	...
	class ItemInsertDialog extends JDialog implements ActionListener
	{
		...
		public ItemInsertDialog(JFrame parent)
		{
			...
			// using the GridBag layout purchase
	...
	{
		String[] receiptID = purchase.findArray("RIDwithoutDD");	// Use SQL to pull receiptIDs that currently don't have a delivered date.
...
//				}
				...
				if (purchase.setDeliveryDate( Integer.valueOf(id)))
				{
					AMS.updateStatusBar("Operation successful.");

					String dateOfPurchase = purchase.findInfo(Integer.valueOf(id), "dop");
					purchaseDate.setText(dateOfPurchase);

					String cname = purchase.findInfo(Integer.valueOf(id), "cname");
	...
	class TopSalesItemsReportDialog extends JDialog implements ActionListener
	{
		...
		//					{
		//						Toolkit.getDefaultToolkit().beep();
		//						AMS.updateStatusBar("Purchase " + cid.toString() + " already exists!");
		//						return OPERATIONFAILED; 
		//					}
		//				}
		//				else
		...
		//
		//				if (purchase.addItem(cid, password, name, address, phone))
		//				{
		//					AMS.updateStatusBar("Operation successful.");
		//					return OPERATIONSUCCESS; 
		//				}
