...
public class BranchController implements ActionListener, ExceptionListener
{
	...
	public BranchController(AMSView AMS)
	{
		this.AMS = AMS;
	...
	public void actionPerformed(ActionEvent e)
	{
		...
		{
			BranchInsertDialog iDialog = new BranchInsertDialog(AMS);
			iDialog.pack();
		...
		{
			BranchDeleteDialog dDialog = new BranchDeleteDialog(AMS);
			dDialog.pack();
			AMS.centerWindow(dDialog);
		...
		private int validateInsert()
		{
			try
			{
				...
				if (branchID.getText().trim().length() != 0)
				{
					...
					{
						Toolkit.getDefaultToolkit().beep();
						AMS.updateStatusBar("Branch " + bid.toString() + " already exists!");
				...
				AMS.updateStatusBar("Inserting branch...");
		...
		private int validateUpdate()
		{
			try
			{
				...
				if (branchID.getText().trim().length() != 0)
				{
					...
					{
						Toolkit.getDefaultToolkit().beep();
						AMS.updateStatusBar("Branch " + bid + " does not exist!");
						return OPERATIONFAILED; 
					}
				}
				else
				...
				AMS.updateStatusBar("Updating branch...");
	...
	class BranchDeleteDialog extends JDialog implements ActionListener
	{
		...
		private int validateDelete()
		{
			try
			{
				...
				AMS.updateStatusBar("Deleting branch...");
